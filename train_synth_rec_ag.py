...
IMG_SIZE, DEVICE = 512, torch.device('cuda' if torch.cuda.is_available() else 'cpu')
LAMBDA_IMG, LAMBDA_BG = 10.0, 0.1
EPOCHS, BS, LR, PATIENCE = 20, 4, 3e-4, 8
...
def main():
    ...
    writer = SummaryWriter(f'runs/synth_rec_ag_{run_id}')
    ...
    pr = Path('models/unet_synth_rec.pt')
    if pr.exists():
        ck = torch.load(pr,map_location=DEVICE)
        model.load_state_dict(ck.get('model_state',ck), strict=False)
        print('Loaded synth_rec weights')
    ...
    for ep in range(1,EPOCHS+1):
        ...
        print(f'Ep{ep}/{EPOCHS} | val iou={vlm["iou"]:.4f} psnr_c={vlm["psnr_c"]:.2f} ssim_c={vlm["ssim_c"]:.3f} time={dt:.1f}s')
        writer.add_scalar('synth_rec_ag/iou', vlm['iou'], ep)
        writer.add_scalar('synth_rec_ag/psnr_c', vlm['psnr_c'], ep)
        writer.add_scalar('synth_rec_ag/ssim_c', vlm['ssim_c'], ep)
        for k in stats:
            stats[k].append(ep if k=='epoch' else (lr if k=='lr' else vlm[k]))
        if vlm['iou']>best:
            best=vlm['iou']; wait=0
            save_ckpt(model,opt,scaler,ep,stats,'models/unet_synth_rec_ag.pt')
        else:
            wait+=1
            if wait>=PATIENCE: print('Early stop'); break
        sch.step(vlm['iou'])

    csv_path = f"statistics/synth_rec_ag_stats_{run_id}.csv"
    ...
    writer.close()
    print("Training synth_rec_ag completed.")
